...
import 'schema/announcements2_record.dart';
...
export 'schema/announcements2_record.dart';
...
/// Functions to query Announcements2Records (as a Stream and as a Future).
Future<int> queryAnnouncements2RecordCount({
  Query Function(Query)? queryBuilder,
  int limit = -1,
}) =>
    queryCollectionCount(
      Announcements2Record.collection,
      queryBuilder: queryBuilder,
      limit: limit,
    );

Stream<List<Announcements2Record>> queryAnnouncements2Record({
  Query Function(Query)? queryBuilder,
  int limit = -1,
  bool singleRecord = false,
}) =>
    queryCollection(
      Announcements2Record.collection,
      Announcements2Record.fromSnapshot,
      queryBuilder: queryBuilder,
      limit: limit,
      singleRecord: singleRecord,
    );

Future<List<Announcements2Record>> queryAnnouncements2RecordOnce({
  Query Function(Query)? queryBuilder,
  int limit = -1,
  bool singleRecord = false,
}) =>
...
Future<FFFirestorePage<Announcements2Record>> queryAnnouncements2RecordPage({
  Query Function(Query)? queryBuilder,
  DocumentSnapshot? nextPageMarker,
  required int pageSize,
  required bool isStream,
  required PagingController<DocumentSnapshot?, Announcements2Record> controller,
  List<StreamSubscription?>? streamSubscriptions,
}) =>
    queryCollectionPage(
      Announcements2Record.collection,
      ...
      if (isStream) {
        final streamSubscription =
            (page.dataStream)?.listen((List<Announcements2Record> data) {
          data.forEach((item) {
            final itemIndexes = controller.itemList!
                .asMap()
                .map((k, v) => MapEntry(v.reference.id, k));
